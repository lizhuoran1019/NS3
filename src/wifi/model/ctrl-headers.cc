...
CtrlBAckResponseHeader::SetType (BlockAckType type)
{
  ...
  for (auto& bitmapLen : m_baType.m_bitmapLen)
    {
      BaInfoInstance baInfo;
      baInfo.m_aidTidInfo = 0;
      baInfo.m_startingSeq = 0;
      baInfo.m_bitmap = std::vector<uint8_t> (bitmapLen, 0);
      baInfo.m_ra = Mac48Address ();
      m_baInfo.push_back (baInfo);
    }
}
